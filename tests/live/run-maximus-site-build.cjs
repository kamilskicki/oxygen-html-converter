...
const DEFAULT_REMOTE_FIXTURES = "/tmp/ohc-maximus-fixtures";
const DEFAULT_REMOTE_SCRIPT = "/tmp/ohc-build-maximus-site.php";
const DEFAULT_REMOTE_REPORT = "/tmp/ohc-maximus-site-build-report.json";
...
function log(message) {
  process.stdout.write(`[maximus-site] ${message}\n`);
}
...
function copyInputs(options) {
  ...
  log("Copying build script and Maximus fixtures into container");
  run("docker", [
    "exec",
    options.container,
    "sh",
    "-lc",
    `rm -rf ${shellQuote(options.remoteFixtureDir)} ${shellQuote(options.remoteScript)} ${shellQuote(options.remoteReport)}`,
  ]);
  run("docker", [
    "cp",
...
function runBuild(options) {
  log(`${options.reset ? "Resetting local site and building" : "Building"} Maximus Oxygen site`);
  const args = [
    "exec",
    options.container,
    "php",
  ...
  const output = run("docker", args, { timeout: 600000 });
...
function copyReport(options) {
  ...
  const localPath = path.join(options.outputDir, `maximus-site-build-${stamp}.json`);
