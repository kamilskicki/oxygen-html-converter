...
class CssParserTest extends TestCase
{
    ...
    public function testParseDeclarationsWithSemicolonsInsideValues()
    {
        ...
        $this->assertCount(1, $rules);
        $this->assertArrayHasKey('background-image', $rules[0]['declarations']);
        $this->assertStringContainsString('data:image/svg+xml;utf8', $rules[0]['declarations']['background-image']);
        $this->assertEquals('"a;b"', $rules[0]['declarations']['content']);
        $this->assertEquals('red', $rules[0]['declarations']['color']);
    }
}
