...
HTML;
    ...
    public function testConversionSucceeds(): void
    {
        ...
        // Keep this test side-effect free for stable PHPUnit runs.
        $this->assertIsArray($result['stats']['warnings'] ?? [], 'Warnings should be an array');
    }
    ...
    public function testCssExtraction(): void
    {
        ...
        $this->assertStringContainsString('@keyframes marquee', $result['extractedCss'],
            'Marquee keyframes should be extracted');
        // .animate-marquee and #cursor are consumed as design properties
        // so they may not remain in CSS output
    }
    ...
    public function testCustomClassesTracked(): void
    {
        ...
        // Custom classes that should be detected (not Tailwind patterns)
        // Note: Many classes match Tailwind patterns:
        // - 'animate-marquee' matches animate-* pattern
        // - 'bg-grid' matches bg-* pattern
        // - 'text-outline' matches text-* pattern
        // Only truly custom classes without Tailwind-like prefixes are tracked
        $expectedCustom = ['glitch-hover'];
        ...
        // Verify Tailwind-pattern classes are NOT in custom classes
        $tailwindPatternClasses = ['animate-marquee', 'bg-grid', 'text-outline'];
        foreach ($tailwindPatternClasses as $class) {
            $this->assertNotContains($class, $customClasses,
                "$class matches Tailwind pattern and should not be in customClasses");
        }
    }
    ...
    public function testFormElementsHandled(): void
    {
        ...
        $this->walkTree($result['element'], function($node) use (&$formFound) {
            if (isset($node['data']['type']) && $node['data']['type'] === 'OxygenElements\\HtmlCode') {
        ...
        $this->assertTrue($formFound, 'Form should be converted to HtmlCode element');
    }
    ...
    public function testJavaScriptTransformed(): void
    {
        ...
        $this->walkTree($result['element'], function($node) use (&$jsElements) {
            if (isset($node['data']['type']) && $node['data']['type'] === 'OxygenElements\\JavaScriptCode') {
    ...
    /**
     * Test button elements get proper layout (requires button_centering heuristic)
     */
    public function testButtonLayout(): void
    {
        // Enable button centering heuristic for this test
        ...
        // Check that button has flex layout via class definition
        $uuids = $buttonElement['data']['properties']['meta']['classes'] ?? [];
        $this->assertNotEmpty($uuids, 'Button should have meta.classes UUIDs');

        $classDef = null;
        foreach ($result['classDefinitions'] as $def) {
            if ($def['id'] === $uuids[0]) {
                $classDef = $def;
                break;
            }
        }
        $this->assertNotNull($classDef, 'Button class definition should exist');
    ...
    public function testGridDetection(): void
    {
        ...
        // Find grid elements via class definitions
        $gridElements = [];
        $classDefsById = [];
        foreach ($result['classDefinitions'] as $def) {
            $classDefsById[$def['id']] = $def;
        }

        $this->walkTree($result['element'], function($node) use (&$gridElements, $classDefsById) {
            $uuids = $node['data']['properties']['meta']['classes'] ?? [];
            foreach ($uuids as $uuid) {
                if (isset($classDefsById[$uuid])) {
                    $layout = $classDefsById[$uuid]['properties']['breakpoint_base']['layout'] ?? [];
    ...
    public function testLinkElementTypes(): void
    {
        ...
        $this->walkTree($result['element'], function($node) use (&$textLinks, &$containerLinks) {
            $type = $node['data']['type'] ?? '';
            if ($type === 'OxygenElements\\TextLink') {
                $textLinks[] = $node;
            } elseif ($type === 'OxygenElements\\ContainerLink') {
                $containerLinks[] = $node;
            }
        });
        
        // Should have both types: simple nav links and button-like CTA links
        $this->assertNotEmpty($textLinks, 'Should have TextLink elements for simple links');
    ...
    public function testIdentifyAllIssues(): void
    {
        ...
        $this->walkTree($result['element'], function($node) use (&$issues) {
            ...
            // Element type check removed - converter now uses correct PascalCase names
        ...
        $this->walkTree($result['element'], function($node) use (&$foundTailwindConfig) {
            $type = $node['data']['type'] ?? '';
            if (strpos($type, 'JavaScript') !== false || strpos($type, 'HtmlCode') !== false) {
        ...
        // Keep test deterministic and assertion-driven (no stdout side effects).
